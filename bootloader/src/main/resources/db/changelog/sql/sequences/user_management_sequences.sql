...
create sequence if not exists user_management.user_details_id_seq;
alter sequence user_management.user_details_id_seq owner to postgres;
alter sequence user_management.user_details_id_seq owned by user_management.user_details.id;
SELECT setval(pg_get_serial_sequence('user_management.user_details', 'id'), coalesce(max(id), 0) + 1, false) FROM user_management.user_details;
